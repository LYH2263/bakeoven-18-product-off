...
type P = { id: number; name: string; is_active?: boolean };
...
export default function WindowsPage() {
  ...
  return (<>
    ...
    <div className="toolbar">
      <select value={pid} onChange={e => setPid(Number(e.target.value))}>{products.map(p => <option key={p.id} value={p.id}>{p.name}{p.is_active === false ? "（已停用）" : ""}</option>)}</select>
    </div>
    ...
    <tbody>{rows.map(w => <tr key={w.oven_id}><td>{w.oven_label}</td><td className="mono">{fmt(w.start_min)}–{fmt(w.end_min)}</td><td className="mono">{w.duration_min} min</td></tr>)}
      {!rows.length && <tr><td colSpan={3}>{products.find(p => p.id === pid)?.is_active === false ? "产品已停用，无可用炉位" : "无可用窗口"}</td></tr>}
    </tbody></table>
  </>);
}
